...
const UPDATE_NUMBER = 'ServiceSearchState/UPDATE_NUMBER';
...
To continue the ServiceSearch example, a view might look something like this:
...
const ServiceSearchView = React.createClass({
  ...
  render() {
    ...
    // use reusable components (ActionButton) to dispatch actions created by ServiceSearchState action creators
    return (
      <View style={styles.container}>
        <Text style={styles.counter}>{value}</Text>
        <ActionButton onPress={() => dispatch(ServiceSearchState.increment())} text='+' />
        <ActionButton onPress={() => dispatch(ServiceSearchState.decrement())} text='-' />
      </View>
    );
  }
});
...
Using the ServiceSearch example, the container would be very simple:
...
export default connect(
  state => ({
    value: state.getIn(['serviceSearch', 'value'])
  })
)(ServiceSearchView);
```
